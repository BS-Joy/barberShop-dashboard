...
import SalonBlockListModal from "./SalonBlockListModal";
...
export default function SalonBlockListPage() {
  ...
  if (isLoading) {
    ...
    pageContent =
      salons.length > 0 ? (
        ...
        <EmptyPage emptyText={"No salon blocked yet"} />
      );
  }
  ...
  return (
    <div className={`flex ${salons?.length > 0 ? "" : "justify-center"}`}>
      ...
      <DashboardModal setIsModalOpen={setIsModalOpen} isModalOpen={isModalOpen}>
        <SalonBlockListModal data={modalData} />
      </DashboardModal>
    </div>
  );
}
